...
def update(dt):
   ...
   if game.stage is GameStage.game:
      ...
      if game.lives and not game.player.invulnerable and game.player.collidelist(
            game.asteroids) > -1:
   ...
   if game.stage is GameStage.leader_board and not game.leader_board:
      ...
      game.leader_board['max_leaders'] = sorted(max_leaders.items(), key=operator.itemgetter(1),
                                                reverse=True)[:15]
      game.leader_board['leader_board'] = sorted(leader_board, key=operator.itemgetter(1),
                                                 reverse=True)[:15]
...
def draw():
   ...
   if game.stage is GameStage.start:
      ...
      screen.draw.text('LEVEL %s COMPLETE!' % game.level, midbottom=(WIDTH / 2, HEIGHT / 2),
                       fontsize=60)
      screen.draw.text(str(round(game.score)), midtop=(WIDTH / 2, 10))
      ...
      if game.leader_board:
         ...
         for initial, score in game.leader_board['max_leaders']:
            screen.draw.text('%s %s' % (initial, round(score)),
