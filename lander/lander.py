...
class LandscapeClass:
   ...
   def reset(self):
      ...
      while len(self.landing_spots
                ) < LandscapeClass.n_spots and next_spot_start < LandscapeClass.world_steps:
         next_spot_start += random.randint(10,
                                           50)  # Randomly choose location to start landing spot
         new_landing_spot = LandingSpotClass(
            next_spot_start)  # Make a new landing object at this spot
      ...
      self.world_height.append(random.randint(
         300, 500))  # Start the landscape between 300 and 500 pixels down
      ...
      for star in range(0, LandscapeClass.n_stars):
         ...
         star_y = random.randint(
            0, self.world_height[star_step])  # Keep the stars above the landscape
...
class ShipClass:
   ...
   def update_physics(self):
      ...
      if ship_step < LandscapeClass.world_steps:
         self.altitude = game.landscape.world_height[ship_step] - self.position[1] - (
            LandscapeClass.step_size * 3)
...
def draw():
   ...
   for step in range(0, game.landscape.world_steps - 1):
      ...
      if (game.blink == True
          or game.game_on == False) and game.landscape.get_within_landing_spot(step) == True:
   ...
   if game.blink == True or game.game_on == False:
      for spot in game.landscape.landing_spots:
         ...
         y_text = game.landscape.world_height[
            spot.starting] + 10  # The extra 10 pixels puts the text below the landscape
   ...
   # Draw the stats
   screen.draw.text("SCORE: " + str(round(game.score)), (10, 10), color="white",
                    background="black")
   ...
   screen.draw.text("ALTITUDE: " + str(round(game.ship.altitude)), (WIDTH - 230, 10),
                    color="white", background="black")
   screen.draw.text("HORIZONTAL SPEED: {0:.2f}".format(game.ship.velocity[0]), (WIDTH - 230, 25),
                    color="white", background="black")
   screen.draw.text("VERTICAL SPEED: {0:.2f}".format(-game.ship.velocity[1]), (WIDTH - 230, 40),
                    color="white", background="black")
   ...
   sin_angle = math.sin(
      math.radians(game.ship.angle -
                   45))  # Legs are drawn 45 degrees either side of the ship's angle
   cos_angle = math.cos(math.radians(game.ship.angle - 45))
   screen.draw.line(game.ship.position,
                    (game.ship.position[0] + (sin_angle * size * 3), game.ship.position[1] +
                     (cos_angle * size * 3)), "yellow")
   ...
   if game.ship.booster == True:
      sin_angle = math.sin(math.radians(
         game.ship.angle))  # Booster is drawn at the same angle as the ship, just under it
      cos_angle = math.cos(math.radians(game.ship.angle))
      screen.draw.filled_circle(
         (game.ship.position[0] + (sin_angle * size * 3), game.ship.position[1] +
          (cos_angle * size * 3)), size, "orange")
