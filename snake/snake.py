...
def place_apple():
   ...
   while True:
      pos = (random.randrange(TILES_W), random.randrange(TILES_H))
...
def draw():
   ...
   screen.draw.text('Score: %d' % len(snake), color='white', topright=(WIDTH - 5, 5))

   if not snake.alive:
      screen.draw.text("You died!", color='white', center=(WIDTH / 2, HEIGHT / 2))
